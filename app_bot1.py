...
class TrackGroupChatManager(GroupChatManager):
    def _process_received_message(self, message, sender, silent):
        ...
        with tab1:
            if ( ('UserProxyAgent' in str(sender)) & ~('exitcode' in str(message)) ):            
                with st.chat_message('Requestor', avatar="👨🏻‍💼"):
                    st.markdown(''' :blue[{}]'''.format(message))
            
            elif( ('AssistantAgent' in str(sender)) & ~(  ('fuzzywuzzy' in str(message))  or ('pandas' in str(message)) or ('execution' in str(message)) or ('WO_Nov' in str(message))  )):
...
user_input = st.chat_input("Please provide your name, asset name/type, location, and a brief description of the asset issue....")  
